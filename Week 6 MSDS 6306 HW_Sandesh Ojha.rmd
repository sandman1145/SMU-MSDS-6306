...
df2<- df[-c(2,4,14,8,2,42),]
names(df2)[names(df2) == "Var1"] <- "Statecode"
...
library(ggplot2)
 r <- ggplot(data=df2,aes(x=Statecode,y=Centers,fill=Statecode))+ geom_bar(stat="identity",width= 0.5)+ theme(axis.text.x=element_text(angle=45)) + theme(legend.position = "none")
...
When you combine 2 data frames, the column type has to be the same for the merge to work properly. 
